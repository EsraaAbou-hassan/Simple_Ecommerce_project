...
namespace MVCDotNetV1.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        public ActionResult ProductDetails(int? id)
        {
            ...
            return View(db.Products.Select(x => x.ProductID==id).ToList());
        ...
        public ActionResult Create( Product product,maincat c,subcat s, HttpPostedFileBase Image)
        {
            ...
            ViewBag.maincatID = new SelectList(db.Maincats, "maincatID", "maincatName", product.Subcat.Maincat.maincatID);
            ViewBag.subcatId = new SelectList(db.Subcats, "subcatId", "subcatName", product.Subcat.subcatId);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ProductID,ProductName,ImagePath,Description,Price,Discount,MadeIn,SupplierName,Weight,AvailableQuantity,OrderDetailID,subcatId,Subcat.Maincat")] Product product, subcat s,HttpPostedFileBase Image)
        {
            if (ModelState.IsValid)
            {
                product.subcatId = s.subcatId;
